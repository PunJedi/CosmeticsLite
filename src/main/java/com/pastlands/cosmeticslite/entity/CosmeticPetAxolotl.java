...
/**
 * Cosmetic Pet Axolotl - an axolotl that follows its owner and has cute behaviors.
 * Can survive on land for cosmetic purposes.
 */
public class CosmeticPetAxolotl extends Axolotl {

    // Data for storing owner UUID
    private static final EntityDataAccessor<String> OWNER_UUID = 
            SynchedEntityData.defineId(CosmeticPetAxolotl.class, EntityDataSerializers.STRING);

    public CosmeticPetAxolotl(EntityType<? extends Axolotl> entityType, Level level) {
        super(entityType, level);
        // Set random variant
        RandomSource random = level.random;
        this.setVariant(Axolotl.Variant.values()[random.nextInt(Axolotl.Variant.values().length)]);
    }
    ...
    protected void registerGoals() {
        ...
        // Add cosmetic pet goals - mix of land and water movement
        this.goalSelector.addGoal(1, new FloatGoal(this));
        this.goalSelector.addGoal(2, new WaterAvoidingRandomStrollGoal(this, 1.0D));
        this.goalSelector.addGoal(3, new LookAtPlayerGoal(this, Player.class, 6.0f));
        this.goalSelector.addGoal(4, new RandomLookAroundGoal(this));

        // No attack goals - cosmetic pets don't fight
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (isOwner(player)) {
            if (!this.level().isClientSide) {
                // Axolotls can't sit, so just a friendly interaction
    ...
    public void aiStep() {
        ...
        // Custom follow behavior and land survival
        ...
        if (owner != null && !this.level().isClientSide) {
            ...
            // If moderately far, try to move closer
            else if (distance > 6.0) {
                this.getNavigation().moveTo(owner, 1.0);
            }
        }
        
        // Allow survival on land for cosmetic purposes
