...
/**
 * Cosmetic Pet Fox - a tamed fox that follows its owner but doesn't hunt or breed.
 * Designed to be a purely cosmetic companion.
 */
public class CosmeticPetFox extends Fox {

    // Data for storing owner UUID
    private static final EntityDataAccessor<String> OWNER_UUID = 
            SynchedEntityData.defineId(CosmeticPetFox.class, EntityDataSerializers.STRING);

    public CosmeticPetFox(EntityType<? extends Fox> entityType, Level level) {
        super(entityType, level);
        // Set to red fox by default
        this.setVariant(Fox.Type.RED);
    }
    ...
    protected void registerGoals() {
        ...
        // Remove sit and follow goals since Fox doesn't extend TamableAnimal
        this.goalSelector.addGoal(2, new WaterAvoidingRandomStrollGoal(this, 1.0D));
        this.goalSelector.addGoal(3, new LookAtPlayerGoal(this, Player.class, 8.0f));
        this.goalSelector.addGoal(4, new RandomLookAroundGoal(this));

        // No hunting or targeting goals - cosmetic pets don't fight
    }
    ...
    public void setOwner(@Nullable Player player) {
        if (player != null) {
            // Foxes don't have setTrustedUUID in all versions, just store in our data
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (isOwner(player)) {
            if (!this.level().isClientSide) {
                // Foxes can't sit in this version, just a friendly interaction
