...
/**
 * Cosmetic Pet Chicken - a chicken that follows its owner but doesn't lay eggs or breed.
 * Designed to be a purely cosmetic companion.
 */
public class CosmeticPetChicken extends Chicken {

    // Data for storing owner UUID
    private static final EntityDataAccessor<String> OWNER_UUID = 
            SynchedEntityData.defineId(CosmeticPetChicken.class, EntityDataSerializers.STRING);

    public CosmeticPetChicken(EntityType<? extends Chicken> entityType, Level level) {
    ...
    protected void registerGoals() {
        ...
        // Add cosmetic pet goals only - no FollowOwnerGoal since Chicken isn't TamableAnimal
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        // Only allow the owner to interact (chickens can't sit, so just a friendly interaction)
        if (isOwner(player)) {
            if (!this.level().isClientSide) {
                // Just a friendly bawk
    ...
    @Override
    public void aiStep() {
        // Override to prevent egg laying and add custom follow behavior
        super.aiStep();
        
        // Custom follow behavior since we can't use FollowOwnerGoal
        ...
        // Don't call the egg laying logic
    }
