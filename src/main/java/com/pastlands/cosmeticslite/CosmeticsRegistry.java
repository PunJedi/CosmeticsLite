...
public final class CosmeticsRegistry {
    ...
    public static final String PACK_BLENDED = "blended";

    /**
     * Check if a cosmetic is a blended particle.
     */
    public static boolean isBlendedParticle(CosmeticDef def) {
        if (!TYPE_PARTICLES.equals(def.type())) {
            return false;
        }
        String path = def.id().getPath(); // e.g. "particle/heart_blended"
        if (path.contains("blended")) {
            return true;
        }
    ...
    /** Replace full registry, optionally add dev seed. */
    public static synchronized void replaceAll(Collection<CosmeticDef> defs, boolean addDevSeed) {
        ...
        LOGGER.info("[CosmeticsLite] Registry replaced: {} cosmetics (seed={})", BY_ID.size(), addDevSeed);
    }
    ...
    /** Merge/override properties for an existing def by id. */
    ...
    private static void installDevSeedUnlocked() {
        ...
        seedDevBlendedParticlesUnlocked();
        seedDevHatsUnlocked();
    ...
    private static void discoverCustomHatsFromAssets() {
        try {
            ...
            Map<String, net.minecraft.world.item.Item> CATEGORY_ICONS = Map.of(
				...
				"custom", Items.NAME_TAG,
				"fall", Items.PUMPKIN_PIE
            );
    ...
    // ------------------------------------------------------------------------
    // Dev seed: Blended particle variants
    // ------------------------------------------------------------------------
    private static void seedDevBlendedParticlesUnlocked() {
        // Example blended entries. Use whatever effect names you already have profiles for.
        registerUnlocked(makeBlendedParticleDef(
            "heart_blended",
            "Blended Heart Halo",
            Items.PINK_DYE,
            "heart"
        ...
        registerUnlocked(makeBlendedParticleDef(
            "happy_villager_blended",
            "Blended Emerald Aura",
            Items.EMERALD,
            "happy_villager"
        ));
        ...
        registerUnlocked(makeBlendedParticleDef(
            "shadow_wisps_blended",
            "Blended Shadow Wisps",
            Items.ENDER_PEARL,
            "smoke"
        ));

        registerUnlocked(makeBlendedParticleDef(
            "starfall_crown_blended",
            "Blended Starfall Crown",
            Items.NETHER_STAR,
            "end_rod"
        ));
        ...
        registerUnlocked(makeBlendedParticleDef(
            "bubbling_ground_blended",
            "Blended Bubbling Ground",
            Items.SLIME_BALL,
            "bubble"
        ));
        ...
        registerUnlocked(makeBlendedParticleDef(
            "arcane_spiral_blended",
            "Blended Arcane Spiral",
            Items.END_CRYSTAL,
            "enchant"
        ));

        // Sort blended entries together with the rest of the particles
        // Use the same sorting logic as the UI: non-blended first, then blended
        List<CosmeticDef> particles = BY_TYPE.get(TYPE_PARTICLES);
        if (particles != null) {
            particles.sort(Comparator
                .comparingInt((CosmeticDef def) -> isBlendedParticle(def) ? 1 : 0)
                .thenComparing(def -> def.name().toLowerCase(Locale.ROOT)));
        }
    }

    private static CosmeticDef makeBlendedParticleDef(String idPath,
                                                      String title,
                                                      Item iconItem,
                                                      String baseEffectName) {
        ResourceLocation id = rl("cosmeticslite", "particle/" + idPath);
        ResourceLocation icon = BuiltInRegistries.ITEM.getKey(iconItem);

        // Same underlying Minecraft particle as the base effect
        Map<String, String> props = Map.of("effect", rl("minecraft", baseEffectName).toString());

        // Use pack "blended" so they are grouped logically
        return new CosmeticDef(
            id,
            title,
            "Blended particle: " + title,
            TYPE_PARTICLES,
            icon,
            props,
            PACK_BLENDED
    ...
    // ------------------------------------------------------------------------
    // Dev seed: Gadgets - REMOVED
    // ------------------------------------------------------------------------
    // Gadget seeding has been removed as part of gadget purge
    ...
    // ------------------------------------------------------------------------
    // JSON presets loader - REMOVED
    // ------------------------------------------------------------------------
    // Gadget preset loading has been removed as part of gadget purge
